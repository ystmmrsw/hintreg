...
If the dependent variable has $J+1$ levels,
so that there are $J$ thresholds,
then excluding the thresholds of the indifference limen,
there must be $J-2$ elements
in **threshbelow** and **threshabove** (one of them can be null).
If not null,
the elements in **threshbelow** must be negative,
and those in **threshabove** must be positive,
both in the increasing order.
Identification requires that $J \ge 4$.
Since thresholds are in the increasing order,
**limenlb** must be negative and no less than **max(threshbelow)**,
and **limenub** must be positive
and no greater than **min(threshabove)**.

Internally,
**hintreg()** fits the model by the ML method using **optim()**,
where we use the BFGS method and numerical derivatives for the gradients.
...
1. Gone/Go down
1. Not changed/change (indifference limen)
1. Up by 1\% or less
...
Thus we have interval data with an indifference limen.

The survey dates and variables in **ias2009febmay** are just enough
to replicate Table 3, column (3) in @Blanchflower09.
One can check all variables in the data set by simply summarizing it:
...
@Blanchflower09 implicitly assume
that the indifference limen is $[-.5,.5]$,
and fit a normal interval regression model
...
one can replicate Table 3, column (3) in @Blanchflower09
as follows:
...
We want to extend this model by allowing for conditional heteroskedasticity,
an indifference limen, and a heterogeneous indifference limen.
As an illustration, however,
ML estimation takes too much time for the last case.
So we use **sex** as the only explanatory variable;
i.e., we look at the gender gap in inflation expectations
...
Now we allow for conditional heteroskedasticity.
Since the likelihood function may no longer be unimodal,
it is a good idea to choose the starting values for the parameters carefully,
...
by allowing the threshold equations to have only intercepts.
Here we set the starting values as follows:

conditional mean equation
  : use the estimated coefficients for the conditional mean equation
  in a generalized normal interval regression model

conditional standard deviation equation
  : use the estimated coefficients for the conditional standard deviation equation
  in a generalized normal interval regression model
...
We can fit a heterogenous normal interval regression model
...
## Heterogenous normal interval regression model

Finally, we allow for a heterogeneous indifference limen.
Here we set the starting values as follows:
